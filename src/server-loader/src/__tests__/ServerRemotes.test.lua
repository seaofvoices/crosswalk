...
for methodName, info in pairs(remoteToClientCases) do
    describe(methodName, function()
        ...
        it('creates a new remote', function()
            expect(remoteStorageMock.event).toBeDefined()
            local remote = remoteStorageMock.event :: RemoteEventMock
            expect(remote.ClassName).toEqual(info.remoteClass)
            expect(remote.Name).toEqual('module.process')
        end)
        ...
        if _G.DEV then
            ...
            if info.canThrow then
                ...
                it('catches errors from one player when calling all players', function()
                    ...
                    expect(allCalled).toEqual(false)
                end)
            end
            ...
            if info.remoteClass == 'RemoteFunction' then
                it('collects player results', function()
                    ...
                    expect(results).toEqual(expect.any('table'))
                    expect(results[playerMock].n).toEqual(2)
                    expect(results[playerMock][1]).toEqual('value')
                    expect(results[playerMock][2]).toBe(playerMock)
                end)
            end
        end
    end)
...
for methodName, info in pairs(remoteToServerCases) do
    describe(methodName, function()
        ...
        it('creates a new remote', function()
            ...
            serverRemotes[methodName](serverRemotes, 'module', 'process', function() end, 'High')
        ...
        it('errors if the security level is unknown', function()
            ...
            expect(function()
                serverRemotes[methodName](serverRemotes, 'module', 'process', function() end, 'oof')
            end).toThrow(
        ...
        for describeSecurity, caseInfo in pairs(securityCases) do
            describe(describeSecurity, function()
                ...
                beforeEach(function()
                    ...
                    serverRemotes:setOnUnapprovedExecution(onUnapprovedExecution:getFunctionValue())
                end)
